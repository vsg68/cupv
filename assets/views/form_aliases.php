	...
	<div class='user-form ui-widget ui-corner-all box-shadow'>
		...
		<form id='usersform' action='#' method='post'>
			...
			<h4></h4>
			   <table>
					<tr>
						<td class='formlabel noname'>Алиас:</td>
						<td><input class='ui-corner-all autocomp' type='text' name='alias_name' value='<?= isset($data->alias_name) ? $data->alias_name : '' ?>'  /></td>
					</tr>
					<tr>
						<td class='formlabel noname'>Переадресация:</td>
						<td><input class='ui-corner-all autocomp'  type='text' name='delivery_to' value='<?= isset($data->delivery_to) ? $data->delivery_to : '' ?>' /></td>
					</tr>
					<?php if(isset($is_alias_page)): ?>
					<tr>
